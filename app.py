...
gcs_bucket = "potato-model-v1"
model_path = "models/1"
model_filename = "model.h5"

# Authenticate to Google Cloud (make sure you have the appropriate credentials set up)
# This step is essential to access GCS resources from your Streamlit app.
os.system("gcloud auth application-default login")

# Download the model file from GCS to a local directory
local_model_path = f"./{model_filename}"
os.system(f"gsutil cp gs://{gcs_bucket}/{model_path}/{model_filename} {local_model_path}")
...
# Sidebar options
option = st.sidebar.selectbox("Choose an option", ["Predict a single image", "Batch Upload"])
...
if option == "Predict a single image":
    ...
    if uploaded_file is not None:
        ...
        st.write(f"{predicted_class.replace('Potato___', '')}, Confidence: {confidence}%")
...
if option == "Batch Upload":
    ...
    if uploaded_files and process_button:
        ...
        for file in uploaded_files:
            ...
            batch_results.append({"File Name": file.name, "Predicted Class": predicted_class.replace('Potato___', ''),
        ...
        st.dataframe(batch_df.style.highlight_max(axis=0, subset=["Confidence (%)"]))

        # Generate a CSV file for batch results
        csv_file = batch_df.to_csv(index=False)
        st.download_button("Download CSV", data=csv_file, file_name="batch_results.csv", key="batch_csv")
        ...
        for i in range(num_rows):
            ...
            for j in range(num_columns):
                ...
                if index < num_images:
                    ...
                    st.image(img, caption=f"Predicted: {predicted_class}\nConfidence: {confidence}%",
                             use_column_width=False, width=128)
                else:
                    st.write("<div style='width: 128px;'></div>", unsafe_allow_html=True)
